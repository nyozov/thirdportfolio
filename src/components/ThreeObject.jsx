

import Marquee from "react-fast-marquee";
import { motion } from 'framer-motion'




// passes img url to threejs canvas
export default function ThreeObject() {
  return (
    <motion.div
    initial={{opacity: 0}}
    transition={{duration: 1, delay: 0.1}}
    whileInView={{opacity: 1}}
    
    className="w-screen mb-6">
    <Marquee speed={40} gradient={false} className='w-screen'>
      <div className='w-full grid grid-cols-4 gap-4'>
   
      <img src={'https://miro.medium.com/max/1400/1*UvWOnSlWP7AbbOtSVUeHMg.png'} className='w-[65px] h-[65px]'/>
      <img src={'https://cdn.iconscout.com/icon/free/png-256/ruby-2752084-2284901.png'} className='w-[65px] h-[65px]'/>
      <img src={'https://cdn-icons-png.flaticon.com/512/919/919825.png'} className='w-[65px] h-[65px]'/>
      <img src={'https://res.cloudinary.com/practicaldev/image/fetch/s--qo_Wp38Z--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://dev-to-uploads.s3.amazonaws.com/i/e0nl7ziy1la7bpwj7rsp.png'} className='w-[65px] h-[65px]'/>
      

      </div>

   
    </Marquee>
    <Marquee speed={40} direction="right" gradient={false} >
      <div className='w-full grid grid-cols-4 gap-4 mt-4'>

    <img src={'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRC3rW8XsouaS0ZcaBsSh7Da2J0YO3lioCR-0i9JbhUU37v_5SknvPzXBFHyzsM8tNlS8k&usqp=CAU'} className='w-[65px] h-[65px]'/>
      <img src={'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRnAhzSkKQ0GsVFcCT9qSsaHy_GohBdd0pFDA&usqp=CAU'} className='w-[65px] h-[65px]'/>
      <img src={'https://cdn.dribbble.com/users/3061686/screenshots/17599145/figma-logo-2_4x.png'} className='w-[65px] h-[65px]'/>
      <img src={'data:image/png;base64,'} className='w-[65px] h-[65px]'/>
    

      </div>
      </Marquee>
    </motion.div>
  );
}
